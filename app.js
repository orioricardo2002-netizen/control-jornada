document.addEventListener("DOMContentLoaded", () => {
    ...
    // Pequeño delay para asegurar que la tabla se renderice antes de meter los datos guardados
    setTimeout(() => {
        cargarDatos();
    }, 300);
    ...
    function generarPeriodo() {
        ...
        // Forzamos las horas a las 00:00:00 para evitar errores de desfases de zona horaria (bucle infinito)
        let fecha = new Date(anio, mes - 1, 26, 0, 0, 0);
    ...
    function calcularHorasFila(fila) {
        ...
        // Si solo marca Entrada Mañana y Salida Tarde (jornada intensiva o continuada)
        ...
        fila.querySelector(".totalHoras").textContent = `${horas}:${String(mins).padStart(2, "0")}`;
    ...
    function calcularExtras(fila, minutos) {
        const exceso = minutos - 480; // 480 min = 8 horas
        ...
        // Tu lógica calcula extras por bloques de 30 min hacia arriba
        ...
        fila.querySelector(".horasExtras").textContent = `${horas}:${String(mins).padStart(2, "0")}`;
    }
    ...
    function actualizarTotales() {
        ...
        document.querySelectorAll("#tablaBody tr").forEach(fila => {
            ...
            const [h1, m1] = horas.split(":").map(Number);
            const [h2, m2] = extras.split(":").map(Number);
        ...
        // CORREGIDO: IDs correspondientes exactos al HTML
    ...
    function guardarDatos() {
        ...
        filas.forEach(fila => {
            ...
            datos.push({
                fecha: fila.querySelector(".fecha").innerText.replace(/\n.*/, ""), // Guarda solo la fecha limpia sin el día de la semana
        ...
        localStorage.setItem("periodoRicardo", document.getElementById("periodo").value);
    }
